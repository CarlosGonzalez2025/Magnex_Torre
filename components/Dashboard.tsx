...
import { LayoutDashboard, Map as MapIcon, RefreshCw, Search, Server, Wifi, Radio, AlertTriangle, XCircle, CloudOff, CheckCircle, Database, Bell, History, BarChart3, ClipboardCheck, Calendar, Settings, Users, LogOut } from 'lucide-react';
...
import { SavedAlertsPanel } from './SavedAlertsPanel';
import { Analytics } from './Analytics';
import { Inspections } from './Inspections';
import { RouteSchedules } from './RouteSchedules';
import { MaintenancePanel } from './MaintenancePanel';
import { AlertSoundToggle } from './AlertSoundSettings';
...
export const Dashboard: React.FC = () => {
  const { user, logout, checkPermission, isAdmin } = useAuth();

  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [loading, setLoading] = useState(true);
  const [lastUpdate, setLastUpdate] = useState<Date>(new Date());
  const [activeTab, setActiveTab] = useState<'table' | 'map' | 'alerts' | 'history' | 'saved' | 'analytics' | 'inspections' | 'schedules' | 'maintenance' | 'users'>('table');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[64px] flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="hidden lg:flex flex-col items-end mr-2 border-r border-slate-200 pr-4">
              <span className="text-xs font-semibold text-slate-500">{user?.full_name}</span>
              <span className="text-xs text-slate-400">{user?.role === 'admin' ? 'Administrador' : 'Usuario'}</span>
            </div>
            ...
            <button
              onClick={logout}
              className="p-2 rounded-full hover:bg-red-50 text-red-600 transition-all"
              title="Cerrar sesión"
            >
      ...
      <main className="flex-1 max-w-[98%] 2xl:max-w-[1920px] w-full mx-auto px-3 sm:px-4 lg:px-6 py-6">
        ...
        <div className="flex flex-col gap-4 mb-6 bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="flex flex-wrap bg-slate-100 p-1 rounded-lg gap-1">
            ...
            {/* Tab Usuarios - Solo visible para admins */}
            {isAdmin() && (
              <button
                onClick={() => setActiveTab('users')}
                className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-all ${activeTab === 'users' ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
              >
          ...
          {(vehicles.length > 0 || loading) ? (
             <>
               ...
               {activeTab === 'users' && <UserManagement />}
                ...
                {activeTab !== 'alerts' && activeTab !== 'history' && activeTab !== 'saved' && activeTab !== 'analytics' && activeTab !== 'inspections' && activeTab !== 'schedules' && activeTab !== 'maintenance' && activeTab !== 'users' && filteredVehicles.length === 0 && !loading && (
