...
const parseDmyTimestamp = (raw: string): string | null => {
...
const parseYmdTimestamp = (raw: string): string | null => {
  const normalized = raw.replace(/\./g, '/').replace(/\s+/g, ' ');
...
const parseDayFirstGpsTimestamp = (...values: unknown[]): string => {
  return parseGpsTimestampWithPreference(true, ...values);
};

const parseGpsTimestampWithPreference = (preferDayFirst: boolean, ...values: unknown[]): string => {
...
const fetchFagorViaAPI = async (): Promise<Vehicle[]> => {
  try {
    ...
    return result.data.map((record: any, index: number) => {
      ...
      const lastUpdate = parseDayFirstGpsTimestamp(
        record.UltimaPosicion,
        record.Ultima_Posicion,
        record.Fecha,
        record.FECHA,
